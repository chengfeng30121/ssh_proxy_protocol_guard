...
## 原理
SPPG 通过解析传入连接的 Proxy Protocol 头来获取源 IP 地址，然后通过 socket 将剩下的数据传给 SSHD 服务器，因为是针对 Termux 所写，因此默认 SSHD 日志路径为 `/data/data/com.termux/files/usr/var/auth.log`，默认端口为 8022。
...
## 运行
本项目没有什么特别的依赖，开发环境 Python 3.13，建议运行在 Python 3.10+ 环境，使用前请将 `constants.py` 中的 `DEFAULT_SSHD_LOG_PATH` 修改为 SSHD 日志路径或运行后在 `SSH_Proxy/config.json` 中修改。
