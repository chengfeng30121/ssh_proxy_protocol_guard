    ...
    def parse_v2(client_sock):
        ...
        try:
            ...
            while len(signature) < 12:
                ...
                if not chunk:
                    logger.warning("读取Proxy Protocol签名时连接关闭")
                    return None, None
                signature += chunk

            # 检查签名
            if signature != ProxyProtocolParser.SIGNATURE_V2:
                logger.warning(f"不是有效的Proxy Protocol v2签名: {signature.hex()}")
                # 不是Proxy Protocol，将数据放回缓冲区（通过创建一个临时socket）
                # 注意：socket不支持真正的unread，我们需要特殊处理
                return None, None

            # 2. 读取版本/命令（1字节）
            version_command = client_sock.recv(1)
            if not version_command:
                logger.warning("读取版本/命令时连接关闭")
                return None, None
            ...
            if command != 0x01:
                ...
                # 对于LOCAL命令，跳过地址信息
                # 需要读取剩余头部并返回None表示使用连接地址
                family_protocol = client_sock.recv(1)
                if not family_protocol:
                    return None, None

                addr_len_bytes = client_sock.recv(2)
                if not addr_len_bytes or len(addr_len_bytes) < 2:
                    return None, None

                addr_len = struct.unpack("!H", addr_len_bytes)[0]
                # 跳过地址数据
                if addr_len > 0:
            ...
            family_protocol = client_sock.recv(1)
            if not family_protocol:
                logger.warning("读取地址族/协议时连接关闭")
                return None, None

            fp_byte = family_protocol[0]
            ...
            # 6. 解析地址
            # 注意：地址族和协议是分开的字段
            # address_family: 0x1=IPv4, 0x2=IPv6, 0x3=Unix
            # transport_protocol: 0x1=STREAM (TCP), 0x2=DGRAM (UDP)

            if address_family == 0x01:  # IPv4
    ...
    def parse(client_sock):
        ...
        try:
            ...
            if data[:12] == ProxyProtocolParser.SIGNATURE_V2:
                logger.debug("检测到Proxy Protocol v2签名，尝试解析...")
                # 移除偷看的数据
                client_sock.recv(16)
                # 重新解析，需要读取更多数据
                return ProxyProtocolParser.parse_v2(client_sock)
